...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        long totalTasksSum = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .count();

        long totalDaysCount = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(o -> DAYS.between(o.getCreated(), LocalDate.now()))
                .reduce(0L, (sum, current) -> sum + current);

        double average = (double) totalDaysCount / totalTasksSum;
    ...
    void testAddTaskListAverageWorkingOnTaskUsingAverage() {
        ...
        OptionalDouble avgDaysCount = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .mapToDouble(o -> DAYS.between(o.getCreated(), LocalDate.now()))
                .average();

        //Then
        assertEquals(10, avgDaysCount.getAsDouble());
